...
# ==== URL Aplikasi ====
BASE_URL = os.getenv("BASE_URL", "https://phicamp-dashboard-production.up.railway.app")
...
def init_db():
    ...
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS phicamp_emails (
...
async def check_email(request: Request):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT is_verified FROM phicamp_emails WHERE email = %s", (email,))
    row = cur.fetchone()
    cur.close()
    conn.close()
...
@app.get("/phicampdashboard", response_class=HTMLResponse)
async def dashboard_login(request: Request):
    ...
    <html>
      <head>
        <title>PhiCamp Dashboard Login</title>
        <style>
          body {
          ...
          button { background: #4f46e5; color: white; border: none; padding: 12px 20px; border-radius: 6px; cursor: pointer; font-size: 16px; }
      ...
      <body>
        <div class="login-box">
          <div class="logo">Φ PhiCamp Dashboard</div>
          <h2>🔒 Login Required</h2>
          <form method="post" action="/phicampdashboard">
...
@app.post("/phicampdashboard", response_class=HTMLResponse)
...
async def show_dashboard(page: int = 1, page_size: int = 20):
    ...
    cur.execute("SELECT COUNT(*) FROM phicamp_emails")
    ...
    <html>
      <head>
        <title>PhiCamp Email Dashboard</title>
        <style>
          body {{ font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: #f3f4f6; padding: 20px; }}
          h1 {{ text-align: center; color: #4f46e5; }}
          .logo {{ text-align: center; font-size: 28px; font-weight: bold; color: #4f46e5; margin-bottom: 10px; }}
          ...
          .logout {{ color: #4f46e5; border: 1px solid #4f46e5; }}
          .logout:hover {{ background: #4f46e5; color: white; }}
          .download {{ background: #10b981; color: white; }}
          .download:hover {{ background: #059669; }}
          .buttons {{ text-align:center; margin-top: 20px; }}
          .pagination {{ text-align: center; margin: 20px 0; }}
          .pagination a {{ display: inline-block; padding: 8px 12px; margin: 0 4px; text-decoration: none; border: 1px solid #4f46e5; border-radius: 4px; color: #4f46e5; }}
          .pagination a:hover {{ background: #4f46e5; color: white; }}
          .pagination .current {{ background: #4f46e5; color: white; }}
    ...
    # Previous button
    if has_prev:
        html += f'<a href="/phicampdashboard?page={page-1}">« Previous</a>'
    else:
    ...
    if start_page > 1:
        html += f'<a href="/phicampdashboard?page=1">1</a>'
        if start_page > 2:
            html += '<span>...</span>'
    ...
    for p in range(start_page, end_page + 1):
        if p == page:
            ...
            html += f'<a href="/phicampdashboard?page={p}">{p}</a>'
    
    if end_page < total_pages:
        if end_page < total_pages - 1:
            html += '<span>...</span>'
        html += f'<a href="/phicampdashboard?page={total_pages}">{total_pages}</a>'
    
    # Next button
    if has_next:
        html += f'<a href="/phicampdashboard?page={page+1}">Next »</a>'
    else:
    ...
    html += """
        ...
        <div class="buttons">
            <a href="/phicampdashboard/download" class="download">Download as CSV</a>
            <a href="/phicampdashboard/logout" class="logout">Logout</a>
        </div>
      </body>
    </html>
    """
...
# ==== Dashboard Logout ====
@app.get("/phicampdashboard/logout")
async def dashboard_logout(request: Request):
    request.session.clear()
    return RedirectResponse("/phicampdashboard")

# ==== Download CSV ====
@app.get("/phicampdashboard/download")
async def download_csv(request: Request):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT email, created_at FROM phicamp_emails ORDER BY created_at DESC")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    return StreamingResponse(
        csv_file,
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=phicamp_emails.csv"}
    )
